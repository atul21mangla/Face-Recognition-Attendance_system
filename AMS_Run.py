...
window = tk.Tk()
window.title("Face Recognition Attendance Management System")

window.geometry('1000x600')
window.configure(background='#F0F0F0') # light grey bg colour

# Update fonts and colors for a more modern look
font_large = ('Helvetica', 16, 'bold')
font_small = ('Helvetica', 12)
button_bg = "#4CAF50"  # Green button color
button_hover_bg = "#45a049"  # Hover color for buttons
    ...
    def fill_attendance():
        ...
        if subb == '':
            ...
            def create_csv():
                ...
                csv_name = 'C:/Users/atulm/Desktop/Python Project/Face-Recognition-Attendance-System-main/Attendance/Manually Attendance/'+DB_table_name+'.csv'
            ...
            def attf():
                import subprocess
                subprocess.Popen(
                    r'explorer /select,"C:/Users/atulm/Desktop/Python Project/Face-Recognition-Attendance-System-main\Attendance\Manually Attendance\-------Check atttendance-------"')
...
def subjectchoose():
    def Fillattendances():
        ...
        if time.time() < future:
            if sub == '':
                ...
                root.configure(background='grey80')
                cs = 'C:/Users/atulm/Desktop/Python Project/Face-Recognition-Attendance-System-main' + fileName
    ...
    def Attf():
        import subprocess
        subprocess.Popen(
            r'explorer /select,"C:/Users/atulm/Desktop/Python Project/Face-Recognition-Attendance-System-main\Attendance\-------Check atttendance-------"')
...
def admin_panel():
    ...
    def log_in():
        ...
        if username == 'Atul' and password == 'atulMangla07':
            win.destroy()
    ...
    def show_student_details():
        ...
        cs = 'C:/Users/atulm/Desktop/Python Project/Face-Recognition-Attendance-System-main/StudentDetails/StudentDetails.csv'
        with open(cs, newline="") as file:
            reader = csv.reader(file)
            r = 0
    ...
    # Create Entry fields for username and password with placeholders
    un_entr = tk.Entry(win, width=20, bg="white", fg="gray", font=('times', 23))
    un_entr.insert(0, "Enter username")
    un_entr.bind("<FocusIn>", lambda event: on_focus_in(event, un_entr, "Enter username"))
    un_entr.bind("<FocusOut>", lambda event: on_focus_out(event, un_entr, "Enter username"))
    un_entr.place(x=290, y=55)

    pw_entr = tk.Entry(win, width=20, show="*", bg="white", fg="gray", font=('times', 23))
    pw_entr.insert(0, "Enter password")
    pw_entr.bind("<FocusIn>", lambda event: on_focus_in(event, pw_entr, "Enter password"))
    pw_entr.bind("<FocusOut>", lambda event: on_focus_out(event, pw_entr, "Enter password"))
...
Notification = tk.Label(window, text="All things good", bg="green", fg="white", width=30,
                        height=2, font=('Helvetica', 14,'bold'))
Notification.place(x=350,y=400)


# Function for a rounded button style
def create_button(master, text, command, x, y, width=20, height=2):
    button = tk.Button(master, text=text, command=command, bg=button_bg, fg="white", width=width, height=height,
                       font=font_small, relief="flat", activebackground=button_hover_bg)
    button.place(x=x, y=y)
    return button

# Create labels and buttons with consistent styling
message = tk.Label(window, text="Face-Recognition-Based-Attendance-Management-System", bg="black", fg="white", width=50,
                   height=3, font=('times', 30, ' bold '))
message.place(x=80, y=20)

lbl = tk.Label(window, text="Enter Enrollment : ", width=20, height=2, fg="black", bg="grey", font=font_large)
lbl.place(x=200, y=200)

# Entry field for enrollment with updated styling
txt = tk.Entry(window, validate="key", width=20, bg="white", fg="black", font=('times', 25))
txt.place(x=550, y=210)

clearButton = create_button(window, "Clear", lambda: txt.delete(0, 'end'), 950, 210)
clearButton1 = create_button(window, "Clear", lambda: txt2.delete(0, 'end'), 950, 310)
...
clearButton = create_button(window, "Clear" , clear,950,210)

clearButton1 = create_button(window, "Clear1" , clear,950,310)

AP = create_button(window, "Check Registered Students", admin_panel, 990, 410)

# Buttons with modern UI
takeImg = create_button(window, "Take Images", take_img, 90, 500)
trainImg = create_button(window, "Train Images", trainimg, 390, 500)
FA = create_button(window, "Automatic Attendance", subjectchoose, 690, 500)
quitWindow = create_button(window, "Manually Fill Attendance", manually_fill, 990, 500)
